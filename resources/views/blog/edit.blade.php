            ...
            <div class="card-body">
                <form id="add_form" class="form-horizontal" action="{{ route('blog.update',$blog->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <input type="text" placeholder="Blog Title" id="title" name="title" value="{{ $blog->title }}" class="form-control">
                            @error('title')
                        ...
                        <div class="col-md-9">
                            <select class="form-control aiz-selectpicker" name="category_id" id="category_id" data-live-search="true">
                                @foreach ($blog_categories as $category)
                                    <option value="{{ $category->id }}" @if($blog->category && $category->id == $blog->category->id) selected @endif>
                    ...
                    <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                @if ($blog->banner)
                                    ...
                                    <div id="image-preview-container" class="mt-2">
                                        <img src="{{ asset($blog->banner) }}" alt="Banner Preview" class="img-thumbnail" style="max-width: 200px; max-height: 200px;">
                                        <br>
                                        <button type="button" style="color:black;" class="btn btn-danger mt-2" id="remove-image">Change Image</button>
                                    </div>
                                    <!-- Hide input field if there is an existing image -->
                                    <input type="file" name="banner" id="banner" accept="image/*" style="display: none;" />
                                @else
                                    <!-- Show input field if there is no existing image -->
                        ...
                        <script>
                            $(document).ready(function () {
                                // Set default value for the file input
                                var defaultBannerValue = "{{ $blog->banner }}";
                                if (defaultBannerValue) {
                                    $('#banner').hide();
                                }
                                ...
                                $('#banner').on('change', function () {
                                    ...
                                    if (input.files && input.files[0]) {
                                        ...
                                        reader.onload = function (e) {
                                            $('#image-preview-container').html('<img src="' + e.target.result + '" alt="Banner Preview" class="img-thumbnail" style="max-width: 200px; max-height: 200px;">');
                                        };
